...
class LeaveCommand extends PlayerCommand {
    ...
    async childExec(message, args) {
        Logger.getInstance().verbose('Commands', 1, '[Volume] Volume command receive. Value: "' + args.input + '"');
        ...
        if (isNaN(volume)) {
            Logger.getInstance().verbose('Commands', 1, '[Volume] Invalid volume parameter, must be a number (int or float).', 'yellow');
        ...
        Logger.getInstance().verbose('Commands', 1, '[Volume] Input normalized to: "' + volume + '"');
        ...
        Logger.getInstance().verbose('Commands', 1, '[Volume] Set volume to "' + volume + '".');
